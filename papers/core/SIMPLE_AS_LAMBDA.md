...
$$
\sigma = \left(\frac{\ell_*}{L_H}\right)^{3/2} \quad\Leftrightarrow\quad \ell_* = L_H \sigma^{2/3}.
$$
...
$$
\mathrm{RMS} = \sigma\sqrt{\frac{L_H}{\ell_*}} = \sigma^{2/3} \qquad\text{(order of magnitude; }d=3\text{)}.
$$
...
$$
\mathrm{RMS}(|\gamma-1|) \approx \alpha \sigma\sqrt{\frac{\chi}{\ell_*}}, \qquad \alpha \equiv 2\varepsilon\frac{\rho_X}{\rho_m|\delta_m|}\sim\mathcal{O}(1).
$$
...
$$
\text{mean: same }\Lambda\text{CDM} \sigma=\text{one grain amplitude} \mathrm{RMS}(|\gamma-1|)\sim \sigma^{2/3}
$$
...
Hall-chip translation (see [`TOPOLOGICAL_EDGE_ANALOGY.md`](../side_threads/TOPOLOGICAL_EDGE_ANALOGY.md)):
