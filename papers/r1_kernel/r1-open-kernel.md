...
| Path accumulation $\sqrt{N_{\mathrm{pat}}}$ alone | $N_{\mathrm{pat}}=\chi/\ell_{\ast}$ for Mpc cells, Gpc paths | $\sqrt{N}=\mathcal{O}(10$–$10^{2})$; need $\sim 10^{59}$ |
...
| P6 | Falsifiers are explicit ([`wp5-falsification.md`](../work_packages/wp5-falsification.md)) |

Anything that chooses $\ell_{\ast}$ so that $\sigma$ “enters Euclid” or “matches $R_8$ after looking at S$_8$” fails P2 and is out of scope ([`BOUNDARY.md`](../BOUNDARY.md)).
...
| Input | Output under counting | $\lvert\ell_{\ast}/R_8-1\rvert$ or note |
|:------|:----------------------|:--------------------------------------|
...
Reproducible: `python scripts/r1/r1_open_kernel_scales.py` · identities in `scripts/core/lib_verified.py`.
...
| Move | Why forbidden |
|:-----|:--------------|
| Fit $\ell_{\ast}$ or $r$ to DESI BAO residuals | Turns a null bound into a detection dial |
| Fit $\ell_{\ast}$ to $R_8$ after inspecting S$_8$ | Same dial, different dataset |
| Treat $\ell_{\ast}/L_P\sim 10^{56}$ as a free gain | Undeclared free parameter (amplification gap) |
...
From [`wp5-falsification.md`](../work_packages/wp5-falsification.md), specialised to R1:
...
That channel does not close R1: it does not fix $\ell_{\ast}$ for the DE residual sector. It is listed here only to prevent category errors — a change of terrain, not a substitute for the open kernel. If pursued, it should live under a separate claim boundary.
