...
*Programme note on walls vs open kernel. Not a derivation of \(\ell_*\).*
...
Telescope-band residuals from counting alone require a mesoscopic DE counting cell, not a Planck cell. The ratio \(\ell_*/L_P\sim 10^{56}\) is not a free multiplier; it is the statement that the two cells are different counting objects.
...
This note is not an answer, but the sharpest open question the soft-regime map currently admits — with the lock that neither DESI nor S\(_8\) may be used to *derive* \(\ell_*\), and with the \(d=3\)-specificity of the \(R_8\) proximity stated so it cannot be misread as multi-angle robustness.
