...
| $\lvert\lambda\rvert_{\mathrm{work}}$ | $\lesssim 1.24\times 10^{-4}$ | DESI $\sigma_X<2.5\times 10^{-2}$ (95% CL) map |
...
|:---|:-----------|:-----------|:---------------|:------------------------|:-------------|
| F1 | BAO residual $\sigma_{\mathrm{res}}$ | $\sigma_{\mathrm{free}}\sim 8.5\times 10^{-5}$ | DESI $\sigma_X<2.5\times 10^{-2}$ (95% CL) (sister) | Measured residual $\gg 2.5\times 10^{-2}$ at fixed $\ell_{\ast}=R_{\mathrm{nl}}$ with no derived damping | Residual stays under ceiling with $\ell_{\ast}$ locked |
...
A0–A1 true (residual χ couples locally to δ_m)?
 ...
 ├─ σ_res ≫ 2.5e-2 at fixed ell_*=R_nl, no damping? → F1 KILL
