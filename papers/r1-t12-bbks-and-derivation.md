...
**Date:** July 2026  
**Status:** (A) BBKS numbers executed · (B) derivation **sketch** with explicit postulates — not a finished QFT proof  
**Code:** [`scripts/r1_t12_bbks_peaks.py`](../scripts/r1_t12_bbks_peaks.py) · [`scripts/r1_t1_mechanisms_compute.py`](../scripts/r1_t1_mechanisms_compute.py)
...
\]
\(R_*\) is the **curvature radius of peaks** (scale of the peak tip), not the filter radius \(R\).

### A.2 Results at \(R=R_{\mathrm{nl}}\) (full \(P(k)\), top-hat)

| Quantity | Value |
|:---------|:------|
| Filter \(R_{\mathrm{nl}}\) | \(5.803\,h^{-1}\mathrm{Mpc}=\mathbf{8.610\,\mathrm{Mpc}}\) |
| \(\sigma_0(R_{\mathrm{nl}})\) | \(1.000\) (by construction) |
| \(\gamma_{\mathrm{BBKS}}\) | \(\approx 0.18\) |
| Peak curvature \(R_*\) | \(1.062\,h^{-1}\mathrm{Mpc}=\mathbf{1.576\,\mathrm{Mpc}}\) |
| \(R_*/R_{\mathrm{nl}}\) | \(\approx 0.18\) |
...
| \(R_*\approx 1.58\,\mathrm{Mpc}\) | Curvature of density peaks (sharper) | Closer to group/NP-A arithmetic row — **different object** |
...
For residual **counting of independent domains**, \(R_{\mathrm{nl}}\) (or \(\ell_{\mathrm{sep}}\)) remains the natural cell; \(R_*\) is the substructure scale, not a replacement without a new principle.
...
\quad\text{(or }g\,\chi\,\rho_m\text{ density coupling).}
\]
...
\[
N=\Bigl(\frac{L_H}{R_{\mathrm{nl}}}\Bigr)^{3},\qquad
\sigma=\frac{1}{\sqrt{N}}=\Bigl(\frac{R_{\mathrm{nl}}}{L_H}\Bigr)^{3/2}\approx 8.5\times 10^{-5},
\]
...
| Claim we may publish under this sketch | Claim we may **not** |
|:---------------------------------------|:---------------------|
...
| BBKS splits peak tip vs domain scales | \(\ell_*=R_*\) without a new principle |
...
| ID | Claim | Evidence |
|:---|:------|:---------|
| B1 | At \(R_{\mathrm{nl}}\), \(\sigma_0=1\), \(R_*\approx 1.58\,\mathrm{Mpc}\), \(\gamma_{\mathrm{BBKS}}\approx 0.18\) | script |
