...
I then ask, without fitting, whether the programme’s **mesoscopic grain** ($\ell_{\ast}\sim\mathrm{Mpc}$, $R_8\sim 12\,\mathrm{Mpc}$) points at a **similar redshift class** as the reported transition ($z\sim 0.3$–$0.7$). Geometry: matter–DE equality sits at $z_{\mathrm{eq}}\approx 0.30$; lookback and $\chi(z)$ grow through the same window where path patch counts $N=\chi/\ell_{\ast}$ become large. That is a **shared scale class**, not a derived mechanism for $H_0(z)$.
...
> If the DE residual sector has a mesoscopic counting / correlation cell $\ell_{\ast}$ of structure scale, then **path-integrated** observables (lensing time delays, distance residuals, slip RMS) acquire a depth-dependent bias once $N=\chi(z)/\ell_{\ast}$ is large — i.e. once probes leave the strictly local $H(z)\approx H_0$ regime. That can *look like* a running of inferred $H_0$ with redshift **without** changing early-universe sound horizon physics.
...
| Survive BAO residual ceiling $\sigma_X<2.5\times 10^{-2}$ (95% CL) | Must be checked a posteriori |
...
| Geometry of $\chi(z)/\ell_{\ast}$ | Free $10^{56}$ or DESI-tuned $\ell_{\ast}$ |
