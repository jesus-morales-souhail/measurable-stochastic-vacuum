# External scale candidates for $\ell_{\ast}$: checked, not confirmed
...
**Date:** July 2026  
**Status:** Desk note — this is **not** a derivation of $\ell_{\ast}$  
...
## 1. What $\ell_{\ast}\approx 2.07\,\mathrm{Mpc}$ is (and is not)

I keep coming back to this number, so I want the status plain.

| | |
|:--|:--|
| **What it is** | Algebraic inverse of counting: $\ell_{\ast}=L_H\sigma^{2/d}$ at $d=3$, $\sigma=10^{-5}$ (NP-A) |
| **Exact (code)** | $\ell_{\ast}^{\mathrm{NP\text{-}A}}=2.0646\,\mathrm{Mpc}$ ($H_0=67.4$) |
...
| **Open kernel** (wrong starting cell?) | Mesoscopic $\ell_{\ast}$ so $\sigma$ is already $10^{-5}$–$10^{-4}$ | **Absent (declared)** — not “failed,” **never derived** |

Even if $\ell_{\ast}$ is physical, the programme toys I have run open **residual / slip** at $10^{-4}$–$10^{-3}$, **not** the $\sim 9\%$ Hubble tension.

---

## 3. External candidates (literature anchors, not fits)

| Candidate | Value (order) | $\lvert\ell/\ell_{\ast}^{\mathrm{NP\text{-}A}}-1\rvert$ | Verdict as *independent* confirmation of NP-A |
|:----------|:--------------|:-----------------------------------------------------|:-----------------------------------------------|
...
| $R_8=8/h$ | $\approx 11.87\,\mathrm{Mpc}$ | $\sim 475\%$ vs NP-A; **near** DESI-ceiling $d=3$ $\ell_{\ast}\approx 12.56\,\mathrm{Mpc}$ | Scale class for **another** row (ceiling), not NP-A |
...
3. It does not use “$\ell_{\ast}$ was 2.06, so look for a 2 Mpc object.”
...
| ID | Claim |
|:---|:------|
| E1 | NP-A $\ell_{\ast}=2.0646\,\mathrm{Mpc}$ is counting inverse only |
...
| Non-claim | |
|:----------|:--|
| N-E1 | $\ell_{\ast}$ is the MW–Andromeda distance |
